...
- Cada `important_pending_decision` lleva `decided_by` (`"user"` | `"agent"` | `"agent_after_investigation"` — quién debería tomar la decisión definitiva; no usar `"user"` por defecto) y `confirmation_trigger` (string o null — solo distinto de null cuando la decisión debe confirmarse obligatoriamente antes de una acción irreversible o de alto riesgo: producción, dinero real, borrado de datos; nunca como forma de convertirla en bloqueante). El `final_prompt` debe mencionar explícitamente el `confirmation_trigger` cuando exista — ver DECISIONS.md.
...
- Antes del análisis completo hay un **triaje previo** (`POST /api/triage`, `TRIAGE_SYSTEM_PROMPT`, `max_tokens` bajo): decide solo si es software, su categoría, y si merece preguntar por Claude Code antes de generar nada más (`claude_code_recommended`). Si lo recomienda, la pregunta se muestra ANTES de llamar a `POST /api/generate-prompt` — nunca se genera el análisis completo (decisiones, `final_prompt`) sin esa confirmación cuando el triaje detecta un proyecto de envergadura. Si el usuario dice que no, `POST /api/generate-prompt` recibe `excludeClaudeCode: true`, que instruye a la IA para que no proponga Claude Code ni como principal ni como complementaria. Ver DECISIONS.md antes de fusionar el triaje con la llamada completa o de sustituirlo por una heurística determinista.
